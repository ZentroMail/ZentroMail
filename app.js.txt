...
const app = {
    ...
    async handleGoogleAuth(intent) {
        ...
        const { error } = await supabaseClient.auth.signInWithOAuth({
            provider: 'google',
            options: {
                redirectTo: 'https://yourproject.vercel.app'
            }
        });
    ...
    async checkOAuthRedirect() {
        ...
        // If they just refreshed and already had a session but no redirect mode
        if (!mode) {
        ...
        if (mode === 'login') {
            ...
            // Internal settings account connect
            if (!profile) {
