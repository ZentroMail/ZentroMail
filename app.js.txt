...
const app = {
    ...
    async init() {
        ...
        // 1. Check if we are in an active OAuth redirect flow
        const isRedirect = window.location.hash.includes('access_token') || window.location.hash.includes('error');
        
        if (isRedirect) {
            const intro = this.el('intro-screen');
            if(intro) { intro.classList.remove('active'); intro.classList.add('hidden'); }
            this.showLoader("Authenticating...");
        }

        // 2. Await the session check before deciding on intro
        const handledSession = await this.checkOAuthRedirect();
        ...
        if (!handledSession) {
            ...
            const intro = this.el('intro-screen');
            if(intro) { intro.classList.remove('active', 'fade-out-slow'); intro.classList.add('hidden'); }
        }
    },

    // ==========================================
    // GOOGLE OAUTH LOGIC (STRICT SEPARATION)
    // ==========================================
    async handleGoogleAuth(intent) {
        ...
        // Store intent to enforce rules after redirect
        localStorage.setItem("auth_mode", intent);
        const { error } = await supabaseClient.auth.signInWithOAuth({
            provider: 'google',
            options: {
                redirectTo: 'https://yourproject.vercel.app' // Update with your actual Vercel domain if needed
            }
        });
        if (error) {
    ...
    async checkOAuthRedirect() {
        ...
        // Give Supabase a moment to parse the URL hash into an active session
        ...
        try {
            ...
            // If user just refreshed their dashboard and has no auth_mode set
            if (!mode) {
                if (profile) {
    ...
    async handleAuth(e) {
        ...
        if (errEl) errEl.classList.add('hidden');
        if (!supabaseClient) {
            if (errEl) { errEl.innerText = "Database connection offline.";
        ...
        if (!this.state.isLoginMode) {
            if (!email.endsWith('@zentro.mail')) {
                if (errEl) { errEl.innerText = "Email must end with @zentro.mail";
                errEl.classList.remove('hidden'); }
                return;
            }
            if (!this.validatePasswordStrength(pass, 'auth')) {
                if (errEl) { errEl.innerText = "Password must meet all security requirements.";
                errEl.classList.remove('hidden'); }
                return;
            }
            ...
            if (!validation.isValid) {
                if (phoneErrEl) { phoneErrEl.innerText = validation.errorMsg;
    ...
    async submitVerification() {
        ...
        if (isSignup) {
            if (!this.state.confirmationResult) {
                if (errEl) { errEl.innerText = "Session expired. Please restart the sign-up process.";
                errEl.classList.remove('hidden'); }
                return;
            }
