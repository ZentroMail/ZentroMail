...
const app = {
    ...
    async checkOAuthRedirect() {
        ...
        // 3. FAILSAFE: Prevent infinite loading
        const failsafeTimer = setTimeout(() => {
            this.hideLoader();
        }, 3000);

        try {
            // 1. Handle session on page load (CRITICAL)
            ...
            if (!session) {
                ...
                clearTimeout(failsafeTimer);
                this.hideLoader(); // Stop loading immediately
                return;
            }
            ...
            // We have a session, show a loader while checking the DB
            ...
            // If user just refreshed their dashboard and has no mode set
            if (!mode) {
                clearTimeout(failsafeTimer);
            ...
            // 2. Full Redirect Handling Logic
