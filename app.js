...
// ==========================================
// Using modular CDN imports for zero-build static deployment
...
const firebaseConfig = {
  ...
  authDomain: "zentro-mail-33f54.firebaseapp.com", // MUST stay as firebaseapp.com
...
const app = {
    ...
    async init() {
        ...
        this.injectRecaptchaContainer();
        
        // Pre-warm reCAPTCHA on load
    ...
    injectRecaptchaContainer() {
        ...
        if (!rc) {
            ...
            rc.style.bottom = '20px';
            rc.style.right = '20px';
            rc.style.zIndex = '9999';
    ...
    // Handles 'normal' sized RecaptchaVerifier
    async setupRecaptcha() {
        ...
        try {
            window.recaptchaVerifier = new RecaptchaVerifier(firebaseAuth, 'recaptcha-container', {
                'size': 'normal', 
                'callback': (response) => {
                    console.log("reCAPTCHA solved securely.");
                },
                'expired-callback': () => {
            ...
            console.log("reCAPTCHA fully rendered and ready.");
        } catch (error) {
    ...
    nav(pageId, isBack = false) {
        ...
        if (pageId === 'status-screen' || pageId === 'developer-status-screen') {
    ...
    showSuccessScreen(title, desc, nextActionStr = "app.nav('dashboard-screen')") {
        const titleEl = this.el('success-title');
        const descEl = this.el('success-desc');
        const btn = this.el('success-btn');
        if(titleEl) titleEl.innerText = title;
        if(descEl) descEl.innerText = desc;
        if(btn) btn.setAttribute('onclick', nextActionStr);
        this.nav('dynamic-success-screen');
    },
    ...
    validatePasswordStrength(pw, prefix) {
        ...
        for (const [id, met] of Object.entries(rules)) {
            ...
            if (el) {
                if (met) { el.classList.add('valid'); el.classList.remove('invalid'); } 
    ...
    validatePhone(raw, countryKey, isSubmit = false) {
        ...
        if (countryKey === 'AE') {
            ...
            if (raw.length === 1 && !['5'].includes(raw[0])) hasPrefixError = true; 
        ...
        if (hasPrefixError) return { isValid: false, errorMsg: `Invalid ${conf.name} mobile number` };
        if (isSubmit && raw.length !== conf.length) return { isValid: false, errorMsg: `Invalid ${conf.name} mobile number` };
    ...
    // ==========================================
    // REAL FIREBASE + SUPABASE AUTHENTICATION
    // ==========================================
    async handleAuth(e) {
        e.preventDefault();
        ...
        if (!this.state.isLoginMode) {
            ...
            try {
                ...
                if (errEl) { 
                    errEl.innerText = "Failed to send SMS. Please complete the reCAPTCHA challenge or check your number."; 
    ...
    async submitVerification() {
        ...
        if (isSignup) {
            ...
            try {
                ...
                if (errEl) { 
                    errEl.innerText = "Invalid code or code expired. Please try again."; 
                    errEl.classList.remove('hidden'); 
                }
                this.clearOTP('auth-otp');
            }
    ...
    // --- SUPPORT SYSTEM (SUPABASE CONNECTED) ---
    ...
    async submitPublicSupport(e) {
        ...
        if (typeEl.value === "other" && !otherEl.value.trim()) {
    ...
    closeManageAccounts() { this.el('switch-account-modal').classList.add('hidden'); },
    ...
    toggleServicesList(gridId = 'apple-services-grid', iconId = 'apple-expand-icon') {
        const grid = document.getElementById(gridId);
        const iconContainer = document.getElementById(iconId);
        if (grid && iconContainer) {
            grid.classList.toggle('hidden');
            if (grid.classList.contains('hidden')) {
                iconContainer.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2997ff" stroke-width="1.5"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg>';
            } else {
                iconContainer.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2997ff" stroke-width="1.5"><circle cx="12" cy="12" r="10"></circle><line x1="8" y1="12" x2="16" y2="12"></line></svg>';
            }
        }
    },

    updateStatusTime() {
        const elMain = document.getElementById('status-last-updated');
        const elDev = document.getElementById('dev-status-last-updated');
        const now = new Date();
        const timeString = now.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
        const timeText = `Last updated today, ${timeString} Gulf Standard Time.`;

        if(elMain) elMain.innerText = timeText;
        if(elDev) elDev.innerText = timeText;
    }
};
