...
const app = {
    ...
    async handleAuth(e) {
        ...
        if (!this.state.isLoginMode) {
            ...
            // Extract ONLY digits for accurate E.164 conversion
            ...
            // Format strictly to E.164
            const fullPhoneString = `${conf.code}${rawPhone}`; 

            this.showLoader("Transmitting secure SMS...");
            
            try {
                // 1. ALWAYS clear old verifier before creating a new one
                if (window.recaptchaVerifier) {
                    window.recaptchaVerifier.clear();
                    window.recaptchaVerifier = null;
                }
                
                // CRITICAL FIX: Ensure no legacy DOM elements are interfering with layout
                document.querySelectorAll('.grecaptcha-badge').forEach(el => el.remove());

                // 2. Initialize invisible reCAPTCHA bound to the submit button exactly at execution time
                window.recaptchaVerifier = new RecaptchaVerifier(firebaseAuth, 'auth-submit', {
                    'size': 'invisible'
                });
                ...
                // Clear on ANY error to prevent duplicate crashes
                if (window.recaptchaVerifier) {
                    window.recaptchaVerifier.clear();
                    window.recaptchaVerifier = null;
                }
    ...
    async submitVerification() {
        ...
        if (isSignup) {
            // Guard: Check if confirmationResult exists before confirming
            ...
            try {
                ...
                // ALWAYS clear reCAPTCHA in a finally block to guarantee cleanup
                if (window.recaptchaVerifier) {
                    window.recaptchaVerifier.clear();
                    window.recaptchaVerifier = null;
                }
    ...
    async submitPublicSupport(e) {
        ...
        if (supabaseClient) {
            try {
                // Wrapped Supabase call in try/catch to prevent silent failures
                ...
                // Show success UI ONLY after successful insert
                this.hideLoader();
