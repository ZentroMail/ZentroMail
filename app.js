...
const app = {
    ...
    async init() {
        ...
        if (window.location.hash || window.location.search) {
            ...
            this.finalizeLogin(session.user);
        } else {
            this.nav('landing-screen');
        }

        // Keep state synced across tabs/reloads
    ...
    // ==========================================
    // NAVIGATION LOGIC (CRITICAL FIX)
    // ==========================================
    
    // For .dashboard-view elements ONLY
    showView(viewId) {
        ...
        if (target) {
            ...
            console.error(`CRITICAL: Dashboard view ${viewId} does not exist.`);
            return;
        }
    ...
    // For .page elements ONLY (Landing, Auth, Footer)
    ...
    async checkOAuthRedirect() {
        ...
        const failsafeTimer = setTimeout(() => { this.hideLoader(); }, 3000);
        ...
        try {
            ...
            if (mode === "login") {
                if (!profile) {
                    ...
                    clearTimeout(failsafeTimer);
                    this.hideLoader();
                    this.nav('google-login-error-screen');
                    return;
                }
                localStorage.removeItem("auth_mode");
                clearTimeout(failsafeTimer);
                ...
                if (profile) {
                    ...
                    clearTimeout(failsafeTimer);
                    this.hideLoader();
                    this.showToast("Account already exists. Please log in.");
                    this.navToAuth('signin');
                    return;
                }
                clearTimeout(failsafeTimer);
                this.hideLoader();
                this.nav('google-signup-step-screen');
            }
        } catch (err) {
    ...
    async completeGoogleSignup() {
        const emailInput = this.el('google-zentro-email').value.trim().toLowerCase();
        if (!emailInput.endsWith('@zentro.mail')) {
            this.showToast("Email must end with @zentro.mail");
            return;
        }
        ...
        const { data: existing } = await supabaseClient.from('profiles').select('id').eq('email', emailInput).maybeSingle();
        if (existing) {
            this.hideLoader();
            this.showToast("Email already taken. Please choose another.");
            return;
        }

        await supabaseClient.from('profiles').insert([{ id: user.id, email: emailInput, provider: 'google' }]);
        localStorage.removeItem("auth_mode");
        this.hideLoader();
        this.finalizeLogin(user);
    },
    ...
    // ==========================================
    // AUTHENTICATION SYSTEM (CRITICAL BUG FIX)
    // ==========================================
    validatePasswordStrength(pw, prefix) {
        ...
        for (const [id, met] of Object.entries(rules)) {
            ...
            if (el) {
                met ? el.classList.add('valid') : el.classList.remove('valid');
                met ? el.classList.remove('invalid') : el.classList.add('invalid');
                if (met) passedCount++;
            }
        }
    ...
    navToAuth(mode) {
        this.state.isLoginMode = (mode === 'signin');
        this.updateAuthUI();
    ...
    async handleAuth(e) {
        ...
        // CRITICAL CHECK: Enforce profile checks
    ...
    async sendPasswordResetCode() {
        const email = this.el('forgot-email-input').value.trim();
        if (!email) return;
        this.showLoader("Sending reset link...");
        const { error } = await supabaseClient.auth.resetPasswordForEmail(email);
        this.hideLoader();
        if (error) {
            this.showToast(error.message);
        } else {
            this.el('forgot-step-1').classList.add('hidden');
            this.el('forgot-step-2').classList.remove('hidden');
        }
    },
    ...
    async verifyPasswordResetCode() {
        const email = this.el('forgot-email-input').value.trim();
        let token = '';
        document.querySelectorAll('#forgot-otp .otp-input').forEach(i => token += i.value);
        ...
        if (error) {
            ...
            this.el('forgot-step-2').classList.add('hidden');
            this.el('forgot-step-3').classList.remove('hidden');
        }
    },

    async completePasswordReset() {
        const newPassword = this.el('forgot-new-password').value;
        ...
        if (error) {
            ...
            this.navToAuth('signin');
            this.el('forgot-step-3').classList.add('hidden');
            this.el('forgot-step-1').classList.remove('hidden');
        }
    },

    // ==========================================
    // SUPPORT FORM (UI + FUNCTIONAL FIX)
    // ==========================================
        ...
        if (supabaseClient) {
            try {
                const { error } = await supabaseClient.from("support_tickets").insert([
                    { subject: subject, body: message, type: selectedType }
                ]);
    ...
    async toggleReadStar() {
        ...
        const isSender = msg.sender === this.state.currentUser.email;
        const currentStar = isSender ? msg.starred_sender : msg.starred_sender;
